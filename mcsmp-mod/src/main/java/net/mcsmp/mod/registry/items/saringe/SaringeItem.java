package net.mcsmp.mod.registry.items.saringe;
...
public class SaringeItem extends Item {

//    private final List<Pair<StatusEffectInstance, Float>> statusEffects;

    private static final int MAX_USE_TIME = 32;

    public SaringeItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (playerEntity == null || !playerEntity.getAbilities().creativeMode) {
            if (stack.isEmpty()) {
                return new ItemStack(ModItems.EMPTY_SARINGE);
            }

            if (playerEntity != null) {
                playerEntity.getInventory().insertStack(new ItemStack(ModItems.EMPTY_SARINGE));
            }
        }
